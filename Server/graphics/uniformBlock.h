...
class block {
    ...
    std::map<std::string,GLintptr>      offsets;
	  std::vector<std::unique_ptr<local>> basicStorage;
    std::map<std::string,unsigned int>  storageNums;
    //map of shaderPrograms to a map of var names to locations
	  std::map<shaderProgram*,std::map<unsigned int,GLint>> basicLocations;
  ...
  public:
    ...
    void writeNum(unsigned int num, const std::array<T,N>& data) {
      if(!gx::sharedUniformsOn) {
        this->basicStorage[num].get().write(data.data());
      }
    }
    template<typename T,size_t N>
    void write(const std::string varName,const, const std::array<T,N>& data) {
      ...
      this->writeNum(this->storageNums[varName],std::forward<std::array<T,N>(data));
    }
    void frameUpdate(shaderProgram*);
